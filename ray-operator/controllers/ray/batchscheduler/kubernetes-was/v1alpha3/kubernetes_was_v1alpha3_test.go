...
func buildWorkload(rayCluster *rayv1.RayCluster, minCount int32) *schedulingv1alpha3.Workload {
	return adapter{}.BuildWorkload(rayCluster, minCount).(*schedulingv1alpha3.Workload)
}

func buildPodGroup(rayCluster *rayv1.RayCluster, minCount int32) *schedulingv1alpha3.PodGroup {
	return adapter{}.BuildPodGroup(rayCluster, minCount).(*schedulingv1alpha3.PodGroup)
}

func TestName(t *testing.T) {
	require.Equal(t, "kubernetes-was-v1alpha3", newScheduler(nil).Name())
}
...
func TestBuildPreemptionPolicyFromAnnotation(t *testing.T) {
	...
	tests := []struct {
		name       string
		annotation string
		want       *schedulingv1alpha3.PreemptionPolicy
	}{
		{name: "preempt lower priority", annotation: "PreemptLowerPriority", want: ptr(schedulingv1alpha3.PreemptLowerPriority)},
		{name: "never", annotation: "Never", want: ptr(schedulingv1alpha3.PreemptNever)},
...
func ptr(p schedulingv1alpha3.PreemptionPolicy) *schedulingv1alpha3.PreemptionPolicy {
	return &p
}
...
func assertSkipCleansUp(t *testing.T, makeSkipped func(*rayv1.RayCluster)) {
	...
	getErr := fakeClient.Get(ctx, types.NamespacedName{Name: "test-cluster-cluster", Namespace: rayCluster.Namespace}, &schedulingv1alpha3.PodGroup{})
	assert.True(t, apierrors.IsNotFound(getErr))

	require.NoError(t, scheduler.DoBatchSchedulingOnSubmission(ctx, rayCluster))
	getErr = fakeClient.Get(ctx, types.NamespacedName{Name: rayCluster.Name, Namespace: rayCluster.Namespace}, &schedulingv1alpha3.Workload{})
	assert.True(t, apierrors.IsNotFound(getErr))
}
		...
		},
		{
			...
			handler: func(writer http.ResponseWriter, request *http.Request) {
				if request.URL.Path == "/apis/scheduling.k8s.io/v1beta1" {
					writer.Header().Set("Content-Type", "application/json")
					assert.NoError(t, json.NewEncoder(writer).Encode(metav1.APIResourceList{GroupVersion: "scheduling.k8s.io/v1beta1"}))
					return
				}
				http.NotFound(writer, request)
			},
