...
// These tests exercise the scheduling.k8s.io/v1alpha3 provider and require a
// Kubernetes 1.37+ cluster serving that API with the operator pinned to
// v1alpha3 (--kubernetes-was-target-version=v1alpha3, since the selector would
// otherwise prefer the more mature v1beta1). They are named with a "V1Alpha3"
// segment so the `test-e2e-kubernetes-was-v1alpha3` Makefile target selects them
// with `-run`.

func TestKubernetesWASV1Alpha3_CreatesWorkloadAndPodGroups(t *testing.T) {
	test := With(t)
	g := NewWithT(t)
	...
	workload, err := GetWorkloadV1Alpha3(test, namespace.Name, rayCluster.Name)
	g.Expect(err).NotTo(HaveOccurred())
	...
	clusterPodGroup, err := GetPodGroupV1Alpha3(test, namespace.Name, rayCluster.Name+"-cluster")
	g.Expect(err).NotTo(HaveOccurred())
	...
	g.Eventually(PodGroupV1Alpha3(test, namespace.Name, rayCluster.Name+"-cluster"), TestTimeoutShort).
...
// TestKubernetesWASV1Alpha3_PreemptionPolicy verifies the gang preemption policy
...
// the operator's KubernetesWASPodGroupPreemptionPolicy gate (set by the v1alpha3 overlay)
// AND the cluster-side PodGroupPreemptionPolicy gate; without either, the apiserver
// silently prunes the field.
func TestKubernetesWASV1Alpha3_PreemptionPolicy(t *testing.T) {
	test := With(t)
	g := NewWithT(t)
	...
	g.Expect(*workload.Spec.PodGroupTemplates[0].PreemptionPolicy).To(Equal(schedulingv1alpha3.PreemptLowerPriority))

	LogWithTimestamp(test.T(), "Verifying preemptionPolicy is persisted on the runtime PodGroup")
	podGroup, err := GetPodGroupV1Alpha3(test, namespace.Name, rayCluster.Name+"-cluster")
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(podGroup.Spec.PreemptionPolicy).NotTo(BeNil())
	g.Expect(*podGroup.Spec.PreemptionPolicy).To(Equal(schedulingv1alpha3.PreemptLowerPriority))
}

func TestKubernetesWASV1Alpha3_PodSchedulingGroup(t *testing.T) {
	test := With(t)
	g := NewWithT(t)
...
func TestKubernetesWASV1Alpha3_AutoscalingGangsFloor(t *testing.T) {
	test := With(t)
	g := NewWithT(t)

	namespace := test.NewTestNamespace()

	// Autoscaling cluster with an elastic worker group (min 1, max 3). WAS gangs
	// only the floor: 1 head + 1 minReplica.
...
func TestKubernetesWASV1Alpha3_GangSchedules(t *testing.T) {
	test := With(t)
	g := NewWithT(t)
	...
	_, err = GetWorkloadV1Alpha3(test, namespace.Name, rayCluster.Name)
	g.Expect(err).NotTo(HaveOccurred())
...
// TestKubernetesWASV1Alpha3_ScaleUpPatchesInPlace verifies the relaxed-immutability
...
func TestKubernetesWASV1Alpha3_ScaleUpPatchesInPlace(t *testing.T) {
	test := With(t)
	g := NewWithT(t)
	...
	g.Eventually(func(inner Gomega) {
		w, err := GetWorkloadV1Alpha3(test, namespace.Name, rayCluster.Name)
		inner.Expect(err).NotTo(HaveOccurred())
	...
	g.Eventually(func(inner Gomega) {
		pg, err := GetPodGroupV1Alpha3(test, namespace.Name, rayCluster.Name+"-cluster")
		inner.Expect(err).NotTo(HaveOccurred())
...
func TestKubernetesWASV1Alpha3_Idempotent(t *testing.T) {
	test := With(t)
	g := NewWithT(t)
	...
	g.Eventually(WorkloadsV1Alpha3(test, namespace.Name), TestTimeoutShort).Should(HaveLen(1))
	g.Eventually(PodGroupsV1Alpha3(test, namespace.Name), TestTimeoutShort).Should(HaveLen(1))

	LogWithTimestamp(test.T(), "Verifying resource counts remain stable over time")
	g.Consistently(WorkloadsV1Alpha3(test, namespace.Name), 10*time.Second, time.Second).Should(HaveLen(1))
	g.Consistently(PodGroupsV1Alpha3(test, namespace.Name), 10*time.Second, time.Second).Should(HaveLen(1))
}

func TestKubernetesWASV1Alpha3_OwnerReferenceGC(t *testing.T) {
	test := With(t)
	g := NewWithT(t)
	...
	g.Eventually(PodGroupsV1Alpha3(test, namespace.Name), TestTimeoutShort).Should(BeEmpty())
}
